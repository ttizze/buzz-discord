...
test("home page loads with Buzzcord branding", async ({ page }) => {
  await page.goto("/");
  await expect(
    page.getByRole("main").getByRole("img", { name: "Buzzcord" }),
  ).toBeVisible();
});
...
test("invite requires age and legal consent before opening Buzzcord", async ({
  page,
}) => {
  ...
  const agreementConfirmation = page.getByLabel(
    "I agree to the Buzzcord Terms of Service and Privacy Policy.",
  );
  const acceptInvite = page.getByRole("button", {
    name: "Accept invite in Buzzcord",
  });
  ...
  await page
    .locator("label")
    .filter({
      hasText: "I agree to the Buzzcord Terms of Service and Privacy Policy.",
    })
