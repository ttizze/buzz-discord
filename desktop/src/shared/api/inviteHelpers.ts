 ...
 *
 * URL inputs (`https://`, `http://`, `buzzcord://join`, legacy `buzz://join`) always carry a
 ...
 *  - `buzzcord://join?relay=<wsUrl>&code=<code>` → `{ relayWsUrl, code }`
...
export function parseInviteInput(input: string): ParsedInvite | null {
  ...
  try {
    ...
    // buzzcord://join?relay=...&code=... (plus legacy buzz://)
    // Non-special schemes put the authority in `host`, not `pathname`.
    if (url.protocol === "buzzcord:" || url.protocol === "buzz:") {
